...
def character_ngram_profile(
    ...
    if len(text) < n:
        # Degenerate (too-short) doc: keep a fixed-length, vocab-keyed vector.
        return {ng: 0.0 for ng in vocab} if vocab is not None else {}
...
def pos_ngram_profile(
    ...
    if total == 0:
        # Degenerate doc (no n-grams): keep a fixed-length, vocab-keyed vector.
        return {ng: 0.0 for ng in vocab} if vocab is not None else {}
